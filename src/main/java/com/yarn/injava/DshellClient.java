...
public class DshellClient {

	private static final Log LOG = LogFactory.getLog(DshellClient.class);
	...
	// =============添加自定义参数↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
	private String[] containerJarPaths = new String[] {};

	private String[] containerArchivePaths = new String[] {};
	...
	public static final String JAR_PATH = "ExecJar";
	 ...
	 * 构造方法
	 */
	public DshellClient() throws Exception {
		this(new YarnConfiguration());
	}

	public DshellClient(Configuration conf) throws Exception {
		this(
				"hadoop.yarn.distributedshell.DshellApplicationMaster",
				conf);
	}
	...
	DshellClient(String appMasterMainClass, Configuration conf) {
		...
		// ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
		opts.addOption("container_files", true,
				"The files that containers will run .  Separated by comma");
		opts.addOption("container_archives", true,
				"The archives that containers will unzip.  Separated by comma");
		// ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
		opts.addOption(
				"shell_command",
				true,
				"Shell command to be executed by "
						+ "the Application Master. Can only specify either --shell_command "
						+ "or --shell_script");
		opts.addOption(
				"shell_script",
				true,
				"Location of the shell script to be "
		...
		// "No. of containers on which the shell command needs to be executed");//删除container数量选项置为1
		...
		if (cliParser.hasOption("log_properties")) {
			...
			try {
				Log4jPropertyHelper.updateLog4jConfiguration(
						DshellClient.class, log4jPath);
			} catch (Exception e) {
				LOG.warn("Can not set up custom log4j properties. " + e);
			}
		}
		// 得到命令行中的container_jars和container_archives选项，并赋值给本地变量↓↓↓↓↓↓↓↓↓↓↓↓↓
		if (cliParser.hasOption("container_files")) {
			containerJarPaths = cliParser.getOptionValue("container_files")
					.split(",");
		}
		if (cliParser.hasOption("container_archives")) {
			containerArchivePaths = cliParser.getOptionValue(
					"container_archives").split(",");
		}
		// ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
		...
		if (containerMemory < 0 || containerVirtualCores < 0
			...
			throw new IllegalArgumentException(
					"Invalid no. of containers or container memory/vcores specified,"
							...
							+ containerVirtualCores + ", numContainer="
							+ numContainers);
		}
	...
	public boolean run() throws IOException, YarnException {
		...
		if (!shellScriptPath.isEmpty()) {
			...
			String shellPathSuffix = appName + "/" + appId.toString() + "/"
					+ SCRIPT_PATH;
		...
		// scripts
		env.put(DshellDSConstants.DISTRIBUTEDSHELLSCRIPTLOCATION,
				hdfsShellScriptLocation);
		env.put(DshellDSConstants.DISTRIBUTEDSHELLSCRIPTTIMESTAMP,
				Long.toString(hdfsShellScriptTimestamp));
		env.put(DshellDSConstants.DISTRIBUTEDSHELLSCRIPTLEN,
		...
		if (containerJarPaths.length != 0) {
			for (int i = 0; i < containerJarPaths.length; i++) {
				...
				if (!containerJarPaths[i].isEmpty()) {
					...
					String jarPathSuffix = appName + "/" + appId.toString()
							+ "/" + jarName;
		...
		if (containerArchivePaths.length != 0) {
			for (int i = 0; i < containerArchivePaths.length; i++) {
				...
				if (!containerArchivePaths[i].isEmpty()) {
					...
					String archivePathSuffix = appName + "/" + appId.toString()
							+ "/" + archiveName;
					Path archiveDst = new Path(fs.getHomeDirectory(),
							archivePathSuffix);
